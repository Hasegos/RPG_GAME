...
using Random = UnityEngine.Random; // System과 Unity Engine의 네임페이스에
                                   // 모두 Random이 존재하기 때문에
...
public class BoardManager : MonoBehaviour
{
    ...
    public Count wallCount = new Count(5, 9);   //  스테이지(레벨)에따른 벽의 랜덤 갯수
    public Count foodCount = new Count(1, 5);   //  스테이지(레벨)에따른 음식의 랜덤 갯수
    ...
    private Transform boardHolder;  //  사용할 객체들을 자식으로 받음(보기 편하기 위해서)
    ...
    void InitializeList()
    {
        ...
        //리스트 채우기
        // 열에 1을 뺸 이유는 floor 타일의 가장자리를 남겨두기 위해서(탈출이 불가능해 질 수 있어서)
        for (int x = 1; x < columns - 1; x++)
        {
            for (int y = 1; y < rows - 1; y++)
            {
                // 게임 상에서 벽, 아이템 ,몬스터 등이 있을 수 있는 위치
                // gridPositons 리스트에 추가
    ...
    void LayoutObjectAtRandom(GameObject[] tileArray, int minimum, int maximum)
    {
        ...
        for (int i = 0; i < objectCount; i++)
        {
            ...
            // titleArrary 배열로 부터, 소환할 랜덤 타일을 가져오기
    ...
    public void SetupScene (int level)
    {
        ...
        // gridPosiotions 리스트를 재설정
        InitializeList();

        // 벽 타일을 최소값과 최대값 기준으로 랜덤하게 생성한다.
        LayoutObjectAtRandom(wallTiles, wallCount.minimum, wallCount.maximum);

        // 음식 타일을 최소값과 최대값 기준으로 랜덤하게 생성한다.
        LayoutObjectAtRandom(foodTiles, foodCount.minimum, foodCount.maximum);

        // 현재 레벨을 기준으로 적의 수를 결정한다.
        int enemyCount = (int)Mathf.Log(level, 2f);

        // enemyCount를 기준으로 적을 생성한다.
